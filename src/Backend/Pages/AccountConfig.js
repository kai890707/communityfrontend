...
const Layout = () => {
    ...
    const createAccount = (e) => {
        e.preventDefault();
        var formData = new FormData(document.getElementById("createAccount-form"));
        if(formData.get('password') !== formData.get('re-password') ){
            CustomErrorAlert("重複密碼錯誤，請重新輸入!");
        }
        tokenPostApi('account/createEditor',formData).then(
            (res)=>{
                if(res.status === 1){
                    CustomSuccessAlert("帳號建立成功!")
                    .then(()=>{
                        window.location.reload();
                    })
                    
                }else if(res.status === 0){
                    CustomErrorAlert("此帳號已存在，請嘗試別組帳號!");
                }
                else if(res.status ===2){
                    tokenExpired(res,'login');
                }else{
        ...
        <Container fluid>
            ...
            <Row className="shadow-sm mt-5 p-5 bg-white">
                <Tabs
                                ...
                                <Form onSubmit={handleSubmit} id="updatePassword-form">
                                    ...
                                    <Row>
                                        <InputGroup className="mb-3">
                                            ...
                                            <FormControl type="password" aria-label="Default"  name="after"  aria-describedby="inputGroup-communityFaceBook" />
                                        </InputGroup>
                                    </Row>
                                    <Row>
                                        <InputGroup className="mb-3">
                                            <InputGroup.Text id="inputGroup-communityInstagram"><h2 className="fw-bold fs-4 m-0">再次輸入新密碼</h2></InputGroup.Text>
                                            <FormControl type="password" aria-label="Default" name="re-after" aria-describedby="inputGroup-communityInstagram" />
                                        </InputGroup>
                                    </Row>
                                    <Row >
                    ...
                    <Tab eventKey="profile" title="新增編輯者">
                        <Row className="bg-white p-4">
                            <Col xs={12}>
                                <Form onSubmit={createAccount} id="createAccount-form">
                                <Row>
                                    <InputGroup className="mb-3">
                                        <InputGroup.Text id="inputGroup-communityName"><h2 className="fw-bold fs-4 m-0">名稱</h2></InputGroup.Text>
                                        <FormControl aria-label="Default" aria-describedby="inputGroup-communityName"name="name"  />
                                    </InputGroup>
                                </Row>
                                <Row>
                                    <InputGroup className="mb-3">
                                        <InputGroup.Text id="inputGroup-communityAddress"><h2 className="fw-bold fs-4 m-0">帳號</h2></InputGroup.Text>
                                        <FormControl aria-label="Default" aria-describedby="inputGroup-communityAddress" name="account" />
                                    </InputGroup>
                                </Row>
                                <Row>
                                    <InputGroup className="mb-3">
                                            <InputGroup.Text id="inputGroup-communityFaceBook"><h2 className="fw-bold fs-4 m-0">密碼</h2></InputGroup.Text>
                                            <FormControl type="password" aria-label="Default"  name="password"  aria-describedby="inputGroup-communityFaceBook" />
                                        </InputGroup>
                                    </Row>
                                    <Row>
                                        <InputGroup className="mb-3">
                                            <InputGroup.Text id="inputGroup-communityInstagram"><h2 className="fw-bold fs-4 m-0">再次輸入新密碼</h2></InputGroup.Text>
                                            <FormControl type="password" aria-label="Default"  name="re-password" aria-describedby="inputGroup-communityInstagram" />
                                        </InputGroup>
                                    </Row>
                                <Row>
                                    <InputGroup className="mb-3">
                                        <InputGroup.Text id="inputGroup-communityHost"><h2 className="fw-bold fs-4 m-0">連絡電話</h2></InputGroup.Text>
                                        <FormControl aria-label="Default" aria-describedby="inputGroup-communityHost"  name="phone" />
                                    </InputGroup>
                                </Row>
                                 <Row>
                                    <InputGroup className="mb-3">
                                        <InputGroup.Text id="inputGroup-communitySecretary"><h2 className="fw-bold fs-4 m-0">電子郵件</h2></InputGroup.Text>
                                        <FormControl type="email" aria-label="Default" aria-describedby="inputGroup-communitySecretary"  name="email"/>
                                    </InputGroup>
                                </Row>
                                 <Row >
                                        <Col xs={{ span: 6, offset: 6 }} className="text-end">
                                            <Button variant="outline-success" type="submit">建立</Button>
                                        </Col>
                                </Row>
                                </Form>
                            </Col>
